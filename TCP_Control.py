...
def frontservo_appointed_detection(pos): 
    """Rotate the front servo to the specified angle."""
    pwm_FrontServo.ChangeDutyCycle(2.5 + 10 * pos / 180)
    time.sleep(0.02)  # Allow some time for the servo to reach the position

def leftrightservo_appointed_detection(pos): 
    """Rotate the left/right servo to the specified angle."""
    pwm_LeftRightServo.ChangeDutyCycle(2.5 + 10 * pos / 180)
    time.sleep(0.02)  # Allow some time for the servo to reach the position

def updownservo_appointed_detection(pos):  
    """Rotate the up/down servo to the specified angle."""
...
def tcp_data_parse():
    ...
    # Analyze the control command of the servo head from the upper computer and execute the steering rotation
    # Example: $4WD,PTZ180# The steering gear is rotated to 180 degrees.
    if "$4WD,PTZ" in InputString:
        start_idx = InputString.find("PTZ") + 3
        end_idx = InputString.find("#")
        
        if end_idx > start_idx:
            m_kp = int(InputString[start_idx:end_idx])
            g_frontServoPos = 180 - m_kp
            NewLineReceived = 0
            InputString = ""  # Reset InputString after processing

    # Analyze the colorful searchlight command from the host computer and light the corresponding color
    # Example: $4WD,CLR255,CLG0,CLB0# Colorful lights bright red
    if "CLR" in InputString:
        # Extract red value
        start_idx = InputString.find("CLR") + 3
        end_idx = InputString.find(",CLG")
        if end_idx > start_idx:
            red = int(InputString[start_idx:end_idx])

        # Extract green value
        start_idx = InputString.find("CLG") + 3
        end_idx = InputString.find(",CLB")
        if end_idx > start_idx:
            green = int(InputString[start_idx:end_idx])

        # Extract blue value
        start_idx = InputString.find("CLB") + 3
        end_idx = InputString.find("#")
        if end_idx > start_idx:
            blue = int(InputString[start_idx:end_idx])
